import { Args, ArgsType, Ctx, Field, ID, InputType, Int, ObjectType, Query, Resolver } from "type-graphql";
...
export class GetQuestionStatsQuery {
  ...
  async getQuestionStats(
    ...
    const question: Question = await connection.manager.findOneOrFail(Question, { where: { uuid: input.questionUuid }, relations: ["answers"] });
    ...
    const posted_answers = await connection.manager.find(Posted_Answer, { where: { question: question }, relations: ["question", "answer"] })

    const getSelectedValues = (numericValues: number[]) => {
      // sort numericValues in ascending order
      numericValues.sort((a, b) => a - b)
      const selectedValues: GetQuestionStatsSelectedAnswersChart = { labels: [], count: [], hightestCount: 0, allNumericValues: areAllAnswersValueNumeric }
    ...
    const selectedAnswersChart = getSelectedValues(posted_answers.map(posted_answer => posted_answer.answer.value))
