import { Args, ArgsType, Ctx, Field, ID, InputType, ObjectType, Query, Resolver } from "type-graphql";
...
export class GetRespondent {
  ...
  async getRespondent(
    ...
    const respondent = await connection.manager.findOneOrFail(Respondent, { where: { uuid: input.respondentUuid } });
    return {
      respondent
    };
  }
