...
export class Question_Resolver implements ResolverInterface<Question> {
  @FieldResolver()
  async answers(@Root() root: Question, @Ctx() connection: Connection) {
    const answers = await connection.manager.find(Answer, {
      where: { question: root },
    });
    return answers;
  }
  ...
  @FieldResolver()
  async posted_answers(@Root() root: Question, @Ctx() connection: Connection) {
